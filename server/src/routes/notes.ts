...
import { generateFullNote, type GenerateNoteOptions } from "../services/aiServices.ts" // Import GenerateNoteOptions
...
router.post("/generate", async (req: Request, res: Response) => {
  ...
  const { title, synopsis, audience, tone, length, save, authorId, categoryId } = req.body as {
    title?: string
    synopsis?: string
    audience?: string
    tone?: string
  ...
  try {
    ...
    if (save && defaultCategoryId) {
      ...
      const fallbackTitle = generatedNote
        .split("\n")[0]
        .replace(/^[#>\-*]+\s*/, "")
        .trim()
      ...
      while (!isUnique && attempts < 10) {
        publicShareId = crypto.randomBytes(8).toString("hex")
      ...
      savedEntry = await prisma.entry.create({
        data: {
          ...
          publicShareId: isUnique ? publicShareId : null, // Use generated unique ID
        },
        select: { id: true },
      })
    }
