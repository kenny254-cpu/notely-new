...
export default function RAGUploader() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (docId: string) =>
      fetch(`/admin/rag/doc/${docId}`, { method: "DELETE" }).then((res) => {
  ...
  return (
    <div className="p-6 space-y-8 bg-gray-50 dark:bg-gray-900 min-h-screen rounded-xl">
      <header className="space-y-2">
        <h1 className={`text-4xl font-extrabold ${FUCHSIA_TEXT_CLASS}`}>RAG Knowledge Base Management</h1>
      ...
      <Tabs defaultValue="upload" className="w-full">
        ...
        <TabsContent
          value="upload"
          className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-2xl border dark:border-gray-700"
        >
        ...
        >
          ...
          {isLoading && (
            <div className="flex items-center justify-center h-40 text-fuchsia-600">
              ...
              <span className="text-lg">Loading document list...</span>
            </div>
          )}

          {error && (
            <p className="p-4 bg-red-50 text-red-700 border border-red-300 rounded-lg text-sm">
              Failed to load documents. Please check the server connection.
            </p>
          )}
              ...
              <Table>
                <TableHeader className="bg-gray-100 dark:bg-gray-700">
                  <TableRow className="dark:hover:bg-gray-700">
                    <TableHead className="w-[400px] text-gray-700 dark:text-gray-200 text-sm font-bold uppercase tracking-wider">
                      Title
                    </TableHead>
                    <TableHead className="text-gray-700 dark:text-gray-200 text-sm font-bold uppercase tracking-wider">
                      Source
                    </TableHead>
                    <TableHead className="w-[180px] text-gray-700 dark:text-gray-200 text-sm font-bold uppercase tracking-wider">
                      Date Indexed
                    </TableHead>
                    <TableHead className="text-right text-gray-700 dark:text-gray-200 text-sm font-bold uppercase tracking-wider">
                      Actions
                    </TableHead>
                  </TableRow>
                </TableHeader>
