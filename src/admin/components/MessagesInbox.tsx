...
const fetchMessages = async (): Promise<UserMessage[]> => {
  ...
  if (contentType && contentType.includes("application/json")) {
    ...
    throw new Error(
      "Received non-JSON response. Check if server endpoint `/admin/messages` exists and returns application/json.",
    )
  }
}
...
export default function MessagesInbox() {
  ...
  const replyStatus =
    selectedMessage && selectedMessage.id === selectedId && reply.trim() && !selectedMessage.adminReply ? "Draft" : null
              ...
              >
                <div className="flex justify-between items-start">
                  <span className="text-xs font-medium text-gray-700 flex items-center">
                    User:{" "}
                    <span className="font-bold ml-1">{m.userId ? m.userId.substring(0, 8) + "..." : "Guest"}</span>
                  </span>
                  <span className={`px-2 py-0.5 text-xs font-semibold rounded-full border ${STATUS_COLORS[status]}`}>
                    {status}
                  </span>
                </div>
                <p className="mt-1 text-gray-900 line-clamp-2 font-medium">{m.message}</p>
                <small className="text-gray-500 text-xs">{new Date(m.createdAt).toLocaleDateString()}</small>
              </div>
            )
          })}
        </div>
