...
const MOCK_NOTES: Note[] = [
  {
    ...
    content:
      "We decided to proceed with React and TanStack Query for the frontend. Backend will use Node.js and PostgreSQL. The initial focus is on user authentication and data modeling. Deadline for V1 is end of Q3.",
  ...
  },
  {
    ...
    content:
      "Focus will be on organic content creation (SEO) and a small paid social media push targeting the US and EU markets. Budget is $50k. KPIs include a 15% increase in signups.",
  ...
  },
  {
    ...
    content:
      "1. Backup current database. 2. Spin up new RDS instance (us-east-1). 3. Perform schema validation. 4. Run data synchronization scripts. 5. Update application environment variables. Expected downtime: 30 minutes.",
  ...
  },
  {
    ...
    content:
      "The 'fullName' field has been deprecated in favor of 'firstName' and 'lastName'. Please update all frontend consumers immediately. The old endpoint will be retired on 2026-01-01.",
  ...
  const filteredNotes = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        (note) => note.title.toLowerCase().includes(term) || note.synopsis.toLowerCase().includes(term),
      )
    }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-extrabold tracking-tight flex items-center text-foreground">
